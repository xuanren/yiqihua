...
- (BOOL)validateMaximumNumberOfSelections:(NSUInteger)numberOfSelections
{
    ...
    if (minimumNumberOfSelection <= self.maximumNumberOfSelection) {
        if (numberOfSelections <= self.maximumNumberOfSelection) {
            ...
            UIAlertView *alert = [[UIAlertView alloc]initWithTitle:nil message:[NSString stringWithFormat:@"目前支持最多选%d张照片",self.maximumNumberOfSelection] delegate:nil cancelButtonTitle:@"确定" otherButtonTitles:nil, nil];
            [alert show];
            return NO;
        }
    ...
    if (kind == UICollectionElementKindSectionFooter) {
        ...
        switch (self.filterType) {
            case QBImagePickerControllerFilterTypeNone:
                footerView.textLabel.text = [NSString stringWithFormat:@"%d张图片，%d个视频",
                                             self.numberOfPhotos,
                                             self.numberOfVideos
                                             ];
                break;
            ...
            case QBImagePickerControllerFilterTypePhotos:
                footerView.textLabel.text = [NSString stringWithFormat:@"%d张图片",self.numberOfPhotos];
                break;
                
            case QBImagePickerControllerFilterTypeVideos:
                footerView.textLabel.text = [NSString stringWithFormat:@"%d个视频",
                                             self.numberOfVideos
                                             ];
                break;
        }
