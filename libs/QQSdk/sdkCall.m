...
//    {
        ...
        _oauth = [[TencentOAuth alloc] initWithAppId:appid
                                                andDelegate:self];
    NSDictionary *qqTencentOauth=[[NSUserDefaults standardUserDefaults] objectForKey:kQQAuthData];
    if(qqTencentOauth) {
        _oauth.accessToken=[qqTencentOauth objectForKey:@"AccessTokenKey"];
        _oauth.expirationDate=[qqTencentOauth objectForKey:@"ExpirationDateKey"];
        _oauth.openId=[qqTencentOauth objectForKey:@"UserIDKey"];
    }
    //}
    return self;
}
