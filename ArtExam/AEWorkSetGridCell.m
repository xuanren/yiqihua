...
- (void)configureWithWorkSet:(AEWorkSet *)workSet{
    ...
    if (workSet.imgUrl != nil) {
        [self.imgView setImageWithURL:[NSURL URLWithString:workSet.imgUrl] placeholderImage:placeImag];
        
    }else [self.imgView setImage:placeImag];
    
    self.titleLbl.text = workSet.mydescription;
    self.numLabel.text = [NSString stringWithFormat:@"共%lu张",(unsigned long)[workSet.picArr count]];
}
