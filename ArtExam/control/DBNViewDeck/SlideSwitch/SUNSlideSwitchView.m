...
static const CGFloat kHeightOfTopScrollView = 0.0f;//44.0
...
- (void)initValues
{
    ...
    //[_rootScrollView removeGestureRecognizer:_rootScrollView.panGestureRecognizer];
    ...
    if (_showPageControl) {
        if (!self.pageControl) {
            ...
            if([[UIDevice currentDevice] resolution] == UIDeviceResolution_iPhoneRetina5) {
                h = _rootScrollView.frame.size.height - 30;
            }
            else {
                h = 440;
            }
            _pageControl = [[UIPageControl alloc]initWithFrame:CGRectMake(0,h, self.frame.size.width, 20)];
        }
        
        self.pageControl.numberOfPages = [_viewArray count];
...
- (void)showFirstPage{
    [_rootScrollView setContentOffset:CGPointMake(0, 0)];
}
...
- (int)currIndex {
    CGFloat pageWidth = self.rootScrollView.bounds.size.width;
    float fractionalPage = self.rootScrollView.contentOffset.x / pageWidth;
